...
plotUMAP_Labels <- function(df, dataType = "qualitative", cmap = NULL, covarLabel = "", point_size=0.5, 
  namedColors=FALSE, plotTitle=NULL, colorLims=NULL, na.value="grey35", useRaster=TRUE){
  ...
  if(useRaster){
    p <- ggplot(df, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
      geom_point_rast(size = point_size) +
      theme_BOR() +  # Assuming theme_BOR() is a predefined theme function
  ...
  # Add cluster labels
  p <- p + geom_text_repel(data = centroids, aes(x = UMAP1, y = UMAP2, label = Cluster), 
            colour = "black", size = 6, fontface = "bold",
            box.padding = 0.35, point.padding = 0.5,
            segment.color = 'grey50') # Adjust repel parameters as needed
  ...
  if(!is.null(cmap)){
    if(namedColors){
      p <- p + scale_color_manual(values=cmap, limits=names(cmap), name=covarLabel, na.value=na.value) +
        guides(fill = guide_legend(title=covarLabel), colour = guide_legend(override.aes = list(size=5)))
    } else {
      names(cmap) <- NULL
      if(dataType == "qualitative"){
